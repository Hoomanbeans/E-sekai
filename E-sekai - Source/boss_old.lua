...
function UpdateSwords(dt, swords, plat, gravity, player, king)
        ...
        for i = 9, 10, 1 do
            ...
            if --[[swords[i].phase3 == true or ]]1 == 1 then
                local acceleration = vector2.new(0, 0)
...
function DrawSwords(swords, king)
    if king.die == false then
        for i = 1, 7, 1 do
            love.graphics.setColor(0.5, 0.5, 0.5, swords[i].alpha)
            love.graphics.rectangle("fill", swords[i].position.x, swords[i].position.y, swords[i].size.x, swords[i].size.y)
        end
        for i = 8, 8, 1 do
